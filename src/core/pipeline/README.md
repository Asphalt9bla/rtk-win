...
- **Layer** — a generic transformation (e.g. `decorative`). Each layer lives in
  its own file (`decorative.rs`) and may have a whole-string form (captured) and
  a per-line form (streaming).
